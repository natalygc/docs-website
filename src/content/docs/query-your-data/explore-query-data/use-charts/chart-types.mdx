...
<CollapserGroup>
  ...
  >
    ...
    For line charts, you can connect charts with other New Relic experiences. This will help you dive deeper into the data and uncover root causes to problems you're trying to solve. When you click on points in a chart that you're interested in, you'll see options to view entity-specific information.

    Here's an example:

    1. Open a chart in a dashboard that has information you want to dig into.
    ...
    2. Hover over over the chart line to expose a specific point on the chart.
    ...
    3. Click on the chart point to reveal a contextual menu displaying the entities associated with the data point in the chart.
    ...
    4. Click on the entity that interests you, and then in the flyout menu, click one of the links for that entity (you'll see **Entity summary** and **Logs** links).
    ...
    As you use this feature, keep the following in mind:

    * Entities only appear in the contextual menu if the widget query explicitly defines the entities queried with `entityGuid`, `entity.guid`, `appName`, `entity.name` or `entityName` in the `WHERE` clause (source code).
    * The visualization is not aware of its location within a capability, so connections may be circular until the next version (for example, from APM summary to APM summary).
    * The initial set of actions is limited to APM summary and Logs.
    * The feature is only supported in line charts and stacked area charts.
  ...
  >
    ...
    * For charts with a time range dependent on the present moment (for example, `since 30 minutes ago`), the data in the CSV is generated from the present moment, not from when the chart was initially generated.
    ...
    <Callout variant="important">
       `Export as CSV` doesn't support some queries:
      * `COMPARE_WITH` clause:
        ```
        SELECT uniques(hostname)
        FROM SystemSample COMPARE WITH 1 week ago 
        LIMIT MAX
        ```
      * Timestamps in aggregated methods are not formatted as timestamp. Only `Timestamp` column is formated (case sensitive). 
        ```
        // It's not formatted since Timestamp is an alias not the column name.
        SELECT max(timestamp) as 'Timestamp' from Transaction
        ```
      * 
